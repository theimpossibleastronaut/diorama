/* This file has been modified, GSVPANO is no longer maintained, but don't just
   overwrite it. Thanks! */
var GSVPANO = GSVPANO || {};
	...
	this.load = function (location, callback) {
		...
		_panoClient.getPanoramaByLocation(location, 50, function (result, status) {
			if (status === google.maps.StreetViewStatus.OK) {
				...
				self.originHeading = result.tiles.originHeading;
				self.originPitch = result.tiles.originPitch;
				self.originPitchYaw = result.tiles.originPitchYaw;
				callback(status);
			} else {
